...
void Scriptable::CreateProjectile(const ResRef& spellResRef, ieDword tgt, int level, bool fake)
{
	...
	if (caster) {
		if (spl->Flags & (SF_HOSTILE | SF_BREAK_SANCTUARY) || GetGlobalID() != tgt) {
...
int Scriptable::SpellCast(bool instant, Scriptable* target, int level)
{
	...
	if (actor) {
		...
		if (!(actor->Modified[IE_AVATARREMOVAL] || instant)) {
			...
			actor->casting_sound = spl->CreateCastingSound(duration, gender, actor->Pos);
			EffectQueue glowFx;
			glowFx.SetOwner(actor);
			glowFx.AddEffect(spl->CreateCastingGlow(duration));
			actor->AddEffects(std::move(glowFx));
		}
		...
		if (target && target->Type == ST_ACTOR) {
			((Actor *)target)->AddEffects(std::move(fxqueue));
		} else {
