...
255,255,255,MAX_FEATV,MAX_FEATV,MAX_FEATV,255,255,255,255,999999,5,5,999999,1,1,//8f
...
0,0,0,0,0,0,0,25,25,255,255,255,0,65535,0,0,//cf - 207
...
void Actor::Die(Scriptable *killer, bool grantXP)
{
	...
	if (InParty) {
		...
		if (grantXP && act) {
			...
			if (act->Modified[IE_SEX] == SEX_SUMMON && act->Modified[IE_EA] <= EA_GOODCUTOFF) {
				InternalFlags|=IF_GIVEXP;
...
void Actor::FinishAttack() {
	...
	// don't apply effects if target is immune
	if (damage != 0) {
