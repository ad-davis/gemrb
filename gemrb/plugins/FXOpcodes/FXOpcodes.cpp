...
int fx_attacks_per_round_modifier (Scriptable* /*Owner*/, Actor* target, Effect* fx)
{
	...
	return fx->FirstApply && fx->Parameter2 == MOD_ABSOLUTE ? FX_INSERT_P : FX_PERMANENT;
}
...
int fx_morale_modifier (Scriptable* /*Owner*/, Actor* target, Effect* fx)
{
	...
	return fx->FirstApply && fx->Parameter2 == MOD_ABSOLUTE ? FX_INSERT : FX_APPLIED;
}
...
int fx_move_to_area (Scriptable* /*Owner*/, Actor* target, Effect* fx)
{
	...
	//if target's area is different from target area
	if (target->Area != fx->Resource) {
		//move to area
